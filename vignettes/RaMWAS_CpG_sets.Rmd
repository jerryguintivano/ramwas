...
the set of CpGs at which RaMWAS calculates methylation coverage
...
We also provide detailed instructions for preparing a custom CpG set.

# Downloadable CpG sets

Although it is common to use only CpGs found in the reference genome
we chose to also incorporate SNP information as SNPs can often create or destroy CpGs.
We also exclude CpGs with unreliable coverage estimates due to poor alignment
as indicated by our in-silico experiment (details below).
...
The human CpG sets provided in the table above
account for SNPs which can create and destroy CpGs.
The SNPs are derived from the 1000 Genomes super populations 
...
A CpG set can be constructed quickly from a reference genome with the `getCpGsetCG` function.
The functions can use any genome available in Bioconductor as `BSGenome` class.
...
For a genome with injected SNPs, we provide the function `getCpGsetALL` for also finding CpGs that can be created by injected SNPs.
The example below uses all SNPs from dbSNP144 for selecting CpGs in human genome.
We do NOT advice using all dbSNP144 SNPs, as it causes an abundance of CpGs that almost never occur in the population.
...
Unfortunately, Bioconductor packages with SNP information 
...
need to be processed by `vcftools` command `--counts`, like this one:

> `vcftools --gzvcf ALL.chr22.phase3.vcf.gz --counts --out count_ALL_chr22.txt`

The generated allele count file can then be used to inject SNPs using the function `injectSNPsMAF`.
...
Once a CpG set is generated, it can be saves with `saveRDS` function for use by RaMWAS.
...
decreasing disk space requirements from about 500 GB to 17 GB for human genome.
...
The generated FASTQ files are then aligned to the reference genome with commands like this

> `bowtie2 --local --threads 14 --reorder -x bowtie2ind -U chr1.fastq.gz | samtools view -bS -o chr1.bam`
...
Once the desired CpG set is generated, it can be saves with `saveRDS` function for use by RaMWAS.
