...
usually includes CpGs created by common SNPs.
...
Parameter `filebam2sample` must point to a file with 
information which samples correspont to which BAMs.

The file here defines samples `sample1`, `sample2`, and `sample3`. 
Sample1 is assigned reads from bams `b1sample1` and `b2sample1`.
```{r eval=FALSE}
...
sample4
```
If a sample has only one BAM and the BAM has the same name as the sample, 
the corresponding line can contain just that name (see `sample4` above).
...
must point to a text file which can be read with `read.table`.
If the file has extension ".csv", it is assumed to be comma separated, 
