...
for calling any RaMWAS pipeline function.
...
Files specified by `file*` parameters should be located here,
...
RDS QC files (BAM QC info) after scanning BAMs.
...
temporary files during construction of coverage matrix at Step 3.
...
results of PCA analysis at Step 4.
...
with one BAM file name per line. BAM file names can contain path,
...
The `filebam2sample` parameter allowed bultiple BAMs
contain information about one sample.
...
Parameter `filecpgset` must be a file storing a set of CpG,
in the format described above, saved using `saveRDS` function.
Similarly, parameter `filenoncpgset`, if defined,
must point to a file storing "good" locations away from any CpGs.
...
should contain phenotype and covariates
