...
but can be applied to other methylomic data as well ([see vignette](RW2_matrix.html)).
...
RaMWAS can be installed with the following commands or
via instructions at [github.com](https://github.com/andreyshabalin/ramwas).
...
The CpGs with low coverage can be excluded based on two criteria.
...
The average CpG score for a sample is a function of the number of reads, 
which is determined by the laboratory protocol 
rather than amount of methylation.
The CpG matrix is therefore normalized to have 
the same average CpG score for each sample.
...
RaMWAS can perform principal component analysis (PCA) on the CpG score matrix.

PCA serves two purposes.
First, PCA can be used to capture the main unmeasured sources 
of variation in the data before or after regressing out measured covariates.
The PCs can be used as covaraies in the association analyses.
Second, the large sample loadings of the top PCs can indicate
multidiomenional outlying samples which should be excluded
...
The top MWAS finding saved in the text file are:
```{r topPvMWAS}
