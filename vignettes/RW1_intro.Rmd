...
provides a full set of tools for methylome-wide association studies for enrichment technologies.
...
The function `ramwas0createArtificialData` creates an artificial CpG set and a number of BAM files with aligned reads.

One may choose to change the project directory `dir` to a more convenient location when running this code.
...
A minimal parameter list below points RaMWAS to the files listed above.
...
At step 1 RaMWAS scans all BAM files listed in `filebamlist` and calculates a number of quality control metrics. 
...
The `minfragmentsize` and `maxfragmentsize` parameters are set to the minimum and maximum size of sequenced DNA fragments.

The CpGs are called isolated if they are located at least `maxfragmentsize` basepairs away from all other CpGs.
The distribution of reads around these CpGs is later used for estimation of fragment size distribution.
...
The following directories are created in the project directory:

* `rds_rbam` -- directory with BAM read start locations, one RDS file per BAM.
* `rds_qc` -- directory with BAM quality control metrics, one RDS file per BAM.
...
* `qc/matched_length` - plots showing distribution of the score (`scoretag` parameter), one plot per BAM.

Here is a sample `coverage_by_density` plot from the simulated data. It shows higher average coverage for regions with higher CpG densities. The drop down on the right is usually caused by low levels of methylation in highly CpG dense regions.
![coverage_by_density](coverage_by_density1.png){width=100px}

**Note**

* BAMs can be located in different directories, but must have unique names.
* If a BAM has already been scanned before, it is not processed again.
...
This step requires an extra parameter `filebam2sample`, a file indicating how BAMs are combined into samples.
Each line in `filebam2sample` has information regarding one sample. If sample1 contains reads from bam1, bam2 and bam3, the line should be
...
Let's assume that each our BAM match exacly one sample with the same name. Then `filebam2sample` should simply list bam names.
...
* `Fragment_size_distribution_estimate.pdf` - plot showing both estimated fragment size distribution and the distribution of distances from read starts to isolated CpGs.\
  Poor fit may indicate lab-tachincal problems or incorrect selection of `minfragmentsize` and `maxfragmentsize` parameters.
...
* The directories listed above contain
    ...
    - PDF files with various QC plots, one page per BAM/sample.
