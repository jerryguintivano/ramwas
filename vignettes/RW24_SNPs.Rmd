...
As methylation (measured with MBD-seq) can occur only at CpGs,
SNPs can directly affect the number of CpGs in a neighborhood
and thus affect methylation measurements.
...
For SNP-corrected analysis RaMWAS requres the usual input
as for steps 4 and 5 with additional SNP matrix.

The SNP data must have the same dimensions as the coverage matrix,
with SNPs matching the CpGs.
...
## Association testing with and without correction for SNPs
