---
title: "Analyzing Data from Other Sources"
...
vignette: >
  %\VignetteIndexEntry{Analyzing data from other sources}
...
such as gene expression, genotype, etc.
...
It must have two columns indicating chromosome number and location 
(`chr` and `position`).
...
Save locations in a filematrix along with text file with chromosome names.\
The filematrix `CpG_locations` must contain two columns - 
chromosome and position, both with integer values.
The chromosome names in `CpG_chromosome_names.txt` are the factor levels
for the chromosome column in the location filematrix.
```{r locSave}
...
The p-values for these correlations and R^2^ show top two PCs are correlated with
...
Let's test for association between
...
pfull = parameterPreprocess(param)
toptbl = read.table(paste0(pfull$dirmwas,'/Top_tests.txt'), 
                    header = TRUE, sep = '\t',
