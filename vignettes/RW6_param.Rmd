...
By default `dirproject` is set to current directory.

The `dirbam` directory is the location where RaMWAS expects to find BAM files.
If it is not an absolute path, it is considered to be relative to `dirproject`.
...
For more on CpG sets see [the CpG set vignette](RW4_CpG_sets.html)

### File with covariates

The parameter `filecovariates`, if defined,
must point to a file containing covariates
